...
def test_02(open_login_page):
    ...
    assert login_page._is_enter_button_displayed() is True, "User was not logged in!"


def test_03(cabinet_page_view):
    cabinet_page = cabinet_page_view
    assert cabinet_page._is_displayed() is True, 'User is unable to see cabinet functions'


def test_04(log_out_from_cabinet):
    log_out = log_out_from_cabinet
    assert log_out._is_displayed() is True, "User was not logged out!"
